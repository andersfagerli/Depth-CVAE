...
def to_numpy(elements):
    ...
    if type(elements) == tuple or type(elements) == list:
        return [x.detach().numpy() if isinstance(x, torch.Tensor) else x for x in elements]
    return elements.detach().numpy() if isinstance(elements, torch.Tensor) else elements
