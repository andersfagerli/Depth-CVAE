...
from depthcvae.utils.metriclogger import MetricLogger
from depthcvae.utils import torch_utils
from depthcvae.trainer.weighting import weigh, beta
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for iteration, (images, targets, image_id) in enumerate(data_loader, start_iter):
                ...
                if cfg.TRAINER.LOG_STEP > 0 and iteration % cfg.TRAINER.LOG_STEP == 0:
                    ...
                    summary_writer.add_scalar('parameters/beta', beta(global_step, milestone), global_step=global_step)
                ...
                if cfg.TRAINER.EVAL_STEP > 0 and global_step % cfg.TRAINER.EVAL_STEP == 0:
                    model.eval()
                    eval_results = do_evaluation(cfg, model, iteration=global_step)
                    model.train()
